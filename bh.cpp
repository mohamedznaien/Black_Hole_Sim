...
struct BlackHole {
    ...
    void draw() {
        int rings = 50;
        double maxRadius = r_s * 1.5;  // Photon sphere
        
        for (int ring = rings; ring >= 0; ring--) {
            double outerR = maxRadius * (double(ring) / rings);
            double innerR = maxRadius * (double(ring - 1) / rings);
            if (innerR < 0) innerR = 0;
            
            // Color gradient: orange outside -> red -> dark red -> black center
            float t = float(ring) / float(rings);
            float r, g, b;
            
            if (t > 0.6f) {
                // Orange to yellow (outer region)
                float s = (t - 0.6f) / 0.4f;
                r = 1.0f;
                g = 0.3f + 0.5f * s;
                b = 0.0f;
        ...
        for (int i = 0; i <= 100; i++) {
            float angle = 2.0f * M_PI * i / 100;
            float x = r_s * 0.65 * cos(angle);
            float y = r_s * 0.65 * sin(angle);
            glVertex2f(x, y);
        }
        glEnd();
    }
...
struct Ray{
    ...
    void draw(const std::vector<Ray>& rays) {
        ...
        // draw each trail as solid white lines
        glLineWidth(2.0f);
...
int main () {
    auto spawnRays = []() {
        ...
        rays.push_back(Ray(vec2(-1e11, 3.27606302719999999e10 - 1e9), vec2(c, 0.0)));
        rays.push_back(Ray(vec2(-1e11, 3.27606302719999999e10 + 1e9), vec2(c, 0.0)));
    };
    ...
    while(!glfwWindowShouldClose(engine.window)) {
        ...
        // R key - restart
        if (glfwGetKey(engine.window, GLFW_KEY_R) == GLFW_PRESS) {
            spawnRays();
            paused = false;
        }
        
        // Space key - toggle pause (with debounce)
        static bool spaceWasPressed = false;
        bool spacePressed = glfwGetKey(engine.window, GLFW_KEY_SPACE) == GLFW_PRESS;
